"""Pętla wariantów ascent, descent i random_same_norm."""
...
from src.score.constants import CONTROL_INTERVENTION_TYPES, INTERVENTION_BATCH_SIZE
from src.score.density import interpolate_score_state
from src.score.intervention import (
    build_full_cap_delta,
    build_random_same_norm_delta,
...
def run_score_control_policy(
    *,
    ...
    """Wykonaj trzy sparowane warianty dla zaakceptowanego kroku ascent."""
    ...
    with tqdm(total=total_steps, desc=f"{eval_split_name} score-control sweep") as progress:
        for feature_name, layer_number in selected_pairs:
            ...
            for start in range(0, len(eval_rows), INTERVENTION_BATCH_SIZE):
                ...
                accepted = selected["accepted_intervention"].astype(bool)
                if bool(accepted.any()):
                    deltas = {
                        "ascent": selected["delta"],
                        "descent": (-selected["delta"]).detach(),
                        "random_same_norm": build_random_same_norm_delta(
                            selected["delta"],
                            intervention_mask=accepted,
                        ),
                    }
                    branch_stats: dict[str, dict[str, object]] = {
                        "ascent": {
                            ...
                            **compute_delta_norm_stats(token_hidden, delta=deltas["ascent"]),
                        }
                    }
                    for intervention_type in ("descent", "random_same_norm"):
                        candidate = evaluate_candidate_delta(
                            token_hidden,
                            delta=deltas[intervention_type],
                            feature_name=feature_name,
                            layer_index_0based=layer_number - 1,
                            maybe_apply_final_norm=maybe_apply_final_norm,
                            lm_head_weight=lm_head_weight,
                        ...
                        branch_stats[intervention_type] = {
                            "steered_feature_value_local": candidate["feature_value"],
                            "steered_score_value_local": candidate["score_value"],
                            "steered_supported": candidate["supported"],
                            "steered_region_label": candidate["region_label"],
                            **compute_delta_norm_stats(token_hidden, delta=deltas[intervention_type]),
                        }

                    branch_outputs = {
                        intervention_type: run_single_intervention_forward(
                            batch=batch,
                    ...
                    for batch_index in np.where(accepted)[0]:
                        ...
                        for intervention_type in CONTROL_INTERVENTION_TYPES:
                            rows.append(
                                build_output_row(
                                    ...
                                    branch_stats=branch_stats[intervention_type],
                                    accepted_step_scale=selected["accepted_step_scale"],
                                    outputs=branch_outputs[intervention_type],
