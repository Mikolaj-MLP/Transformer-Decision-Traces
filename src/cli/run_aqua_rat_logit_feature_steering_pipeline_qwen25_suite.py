...
DEFAULT_SIZES = ["1.5B", "3B", "7B"]


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the AQuA-RAT logit-feature steering pipeline for a Qwen 2.5 Instruct model suite."
    )
    parser.add_argument(
        "--sizes",
        type=str,
        default=",".join(DEFAULT_SIZES),
        help="Comma-separated Qwen2.5 Instruct sizes to run, for example: 1.5B,3B,7B",
    )
